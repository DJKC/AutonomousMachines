...
void move_to_floor(int current_floorr, Floors destination);
...
int check_direction(int floor_user, int floor_current);
//
//
...
void move_to_floor(int current_floorr, Floors destination){
	while(SensorValue[shaft_1] != destination){
		if(check_direction(convert_Floor_to_int(destination), current_floorr) > 0)
			move_motors(15);

		else if(check_direction(convert_Floor_to_int(destination), current_floorr) < 0)
			move_motors(-1 * 20);
	}

	if(SensorValue[shaft_1] == destination)
		move_motors(0);
...
void move_to_bottom(){
	...
	SensorValue[shaft_1] = FIRST;

	return;
}

int check_direction(int floor_user, int floor_current){
	if((floor_user - floor_current) > 0)//moves up a floor
		return 1;

	else if((floor_user - floor_current) == 0)//if moves to same floor
		return 0;

	else if((floor_user - floor_current) < 0)//moves down a floor
		return -1;
